...
(config)#int e1/0/1                                            /*进入需要配置环路检测的端口/*
...
(config-vsf-port1)#vsf port-group int e1/0/25    /*将25端口加入堆叠*/
(config-vsf-port1)#vsf port-group int e1/0/26    /*将26端口加入堆叠*/
(config-vsf-port1)#exit
(config）#vsf priority 1             /* 配置vsf优先级为1（越大越优）*/
(config）#switch convert mode vsf      /* 修改交换机为vsf模式 */

vsf LACP MAD分裂检测(vsf成员都需配置)
(config）#port-group 1           /*创建聚合组1*/
(config）#int port-channel 1     /*进入聚合组1*/
...
(config）#int e1/0/23;2/0/23     /*进入lacp检测端口（二层方向）*/
...
(config-vlan111)#switch int e1/0/24      /* 将两台三层之间用于检测的端口加入检测vlan（需物理连线） */
(config-vlan111)#exit
